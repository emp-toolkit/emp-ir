...
static ZKBoolCircExecPrv<BoolIO<NetIO>> * c_pvr = nullptr;
static ZKProver<BoolIO<NetIO>> * p_pvr = nullptr;
...
void EMP_setup(int party, const char * ip, int threads, int port) {
	...
	c_pvr = new ZKBoolCircExecPrv<BoolIO<NetIO>>();
	p_pvr = new ZKProver<BoolIO<NetIO>>(netios.data(), threads, c_pvr, nullptr);	
}
void EMP_finalize() {
	delete c_pvr;
	delete p_pvr; 
